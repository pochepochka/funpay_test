...
class DatabaseTestMy
{
    ...
    public function testBuildQuery(): void
    {
        ...
        foreach ([null, true] as $block) {
            $results[] = $this->db->buildQuery(
                'SELECT name FROM users WHERE 1{ AND block = ?d} AND ?# IN (?a)',
                [$block ?? $this->db->skip(), 'user_id', [1, 2, 3]]
            );
        }
        ...
        $correct = [
            ...
            'SELECT name FROM users WHERE 1 AND `user_id` IN (1, 2, 3)',
            'SELECT name FROM users WHERE 1 AND block = 1 AND `user_id` IN (1, 2, 3)',
        ];
